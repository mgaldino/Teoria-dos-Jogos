...
Após o jogo, podemos perceber que os jogadores possuem interesses opostos e vocês esperam que o outro não consiga antecipar o que você irá fazer. Nesse caso, ele será indiferente entre jogar qualquer uma de suas estratégias, pois espera que, na média, ambas gerem o mesmo payoff médio.

Uma estratégia mista é justamente um antidoto contra as tentativas dos outros jogadores advinharem o que você irá fazer. Tomemos o jogo de tênis como exemplo. No saque, se seu adversiário antecipar o que você irá fazer, ele terá uma vantagem. Então, você precisa jogar de um jeito que ele não consiga angtecipar ou advinhar suas estratégias.
...
Vamos calcular o payoff (utilidade) esperado das novas estratégias, para poder preencher a nova matriz de payoff aumentada.
...
## Realismo de Estratégias Mistas

É um pouco estranho imaginar que as pessoas irão aleatorizar suas estratrégias como preconizado pelo Equilíbrio de Nash em Estratégias Mistas. No entanto, temos evidênciia de que isso acontece em algumas situações.

Gauriot et. al (2023) estudaram se atletas de tênis sacam o que a teoria prediz. De maneira simplificada, jogadores podem sacas à esquerda ou à direita. Aqui temos um exemplo, com payoffs fictícios, mas refletindo a ideia de que sacar à direita é diferente de sacar à esquerda. Os payoffs representam as probabilidades de ganhar o ponto.


```{r results = "asis", echo=FALSE}
library(knitr)

# Definindo os payoffs
# Ajustando os payoffs para refletir maior probabilidade de vitória ao antecipar o saque
pair <- function(x,y) sprintf("%.2f, %.2f", x,y)
all_pairs <- c(pair(0.58,0.42), pair(0.79,0.21), pair(0.73,0.27), pair(0.49,0.51))
payoff.mat <- matrix(all_pairs, nrow=2, byrow=TRUE)
dimnames(payoff.mat) <- list(c("Saca à Esquerda","Saca à Direita"), c("Recebe à Esquerda", "Recebe à Direita"))


# Exibindo a tabela
kable(payoff.mat, caption = "Matriz de Payoff para o Jogo de Tênis")
```

Para achar os ENEM, precisamos calcular as utilidades esperadas de saque e recebimento (esquerda ou direita) para vcada jogador.

Para A, temos:

\begin{align}
UE(esquerda) = 0.58q+0.79(1−q)=0.79−0.21q \\
 
UE(direita) = 0.73q+0.49(1−q)=0.49+0.24q \\

0.79−0.21q = 0.49+0.24q \\
0.45q = 0.3 \\
q = 30/45 = 1/3
\end{align}


Por raciocínio análogo (verifique), vericaremos que o jogador A deve sacar à esquerda 8/15 vezes e À direita 7/15 vezes. Vejam que, mesmo sua probabilidade de ganahr ser menor na direita quando o outro jogador corretamente antecipa isso, ele deve jogar em equilíbrio quase a mesma percentagem de vezes À direita e À esquerda. Por outro lado, o recebedor deve receber na direita apensas 1/3 das vezes.

Utilizando um modelo como esse, Gauriot et. al (2023) concluíram que jogadores profissionais tendem a sacar como a teoria prediz, efetivamente aleatorizando seu saque na proporção das habilidades dos jogadores, de forma a ter uma probabilidade de vecner o ponto em equilíbrio igual para cada lado da quadra.

As figuras abaixo são respdouzidas tai como aparecem no artigo referenciado. O primeiro gráfico apresenta os saques dos jogadores, em que cada ponto é um saque na quadra. O mais relevante para nós aqui é a comparação entre o segundo e terceiro gráficos, que compara a expectativa teórica com os dados reais de jogadores profissionais e juvenis de Tênis. A linha azul é a espectativa téorica de ENEM, e a linha vermelha os dados reais. Como vemos, tenistas profissionais jogam quase que exatamente ENEM, enquanto jgoadores juvenis, não. Experimento em laboratório mostra também que pessoas comuns têm dificuldade de jogar ENEM.

```{r, bounce-ball, echo=FALSE}
knitr::include_graphics("figure_3_tennis.jpg")
```

```{r men-prob, echo=FALSE}
knitr::include_graphics("mens winning prob.jpg")
```

```{r junior-prob, echo=FALSE}
knitr::include_graphics("junior players.jpg")
```

A minha intrerpretação desse e de outros trabalhos similares é que, quando há muito em jogo, jogadores têm oportunidade de aprender e há seleção dos melhores, estratégias tendem a convergir para o equilíbrio de Nash (se for único).
